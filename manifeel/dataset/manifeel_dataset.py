...
class ManifeelDataset(BaseImageDataset):
    def __init__(self,
        ...
        self.dim_light_factor = dim_light_factor
    ...
    def _sample_to_data(self, sample):     
        ...
        obs_dict = dict()
        excluded_keys = {'left_tactile_camera_taxim', 'right_tactile_camera_taxim'}
        ...
        for key in self.rgb_keys:
            if key in self.shape_meta['obs']:
                ...
                # Apply resizing and light condition (if not excluded)
                resized_image_seq = []
                for image in image_seq:
                    resized = cv2.resize(image, (target_w, target_h))
                    if self.dim_light_factor is not None:
                        if key not in excluded_keys:
                            # Apply lighting change
                            resized = light_condition(resized, factor=self.dim_light_factor)
                    resized_image_seq.append(resized)
                
                resized_image_seq = np.array(resized_image_seq, dtype=np.float32)
...
def light_condition(image, factor=0.1):
