...
from isaacgymenvs.tasks.tacsl.tacsl_env_blocks import TacSLEnvBlocks
...
class TacSLTaskBlocks(TacSLTaskImageAugmentation, TacSLEnvBlocks, FactoryABCTask):
    ...
    def _refresh_task_tensors(self):
        ...
        for idx, keypoint_offset in enumerate(self.keypoint_offsets):
            self.keypoints_gear[:, idx] = torch_jit_utils.tf_combine(self.block_last_quat,
                                                                     self.block_last_pos,
                                                                     self.identity_quat,
    ...
    def _update_rew_buf(self):
        ...
        gear_base_force = self.contact_force_pairwise[:, self.block_last_body_id_env, self.base_body_id_env].clone()
    ...
    def _reset_object(self, env_ids):
        ...
        #     print(f"🔄 Resetting objects for env_ids: {env_ids}")
        
        self._reset_base()
        self._reset_other_parts()
        self._reset_inhand_part(before_move_to_grasp=True)
    ...
    def _reset_other_parts(self):
        ...
        self.block_part1_pos[:, :] = self.base_pos + torch.tensor(
            [0.0, 0.0, 0.002], device=self.device
        )
        self.block_part2_pos[:, :] = self.base_pos + torch.tensor(
            [0.0, 0.0, 0.002], device=self.device
        )
        self.block_sec_last_pos[:, :] = self.base_pos + torch.tensor(
            [0.0, 0.0, 0.002], device=self.device
        )

        # Set block rot
        self.block_part1_quat[:] = self.identity_quat
        self.block_part2_quat[:] = self.identity_quat
        self.block_sec_last_quat[:] = self.identity_quat
        self.block_last_quat[:]  = self.identity_quat

        # Stabilize block
        self.block_part1_linvel[:, :]    = 0.0
        self.block_part2_linvel[:, :]    = 0.0
        self.block_sec_last_linvel[:, :] = 0.0
        self.block_last_linvel[:, :]     = 0.0

        self.block_part1_angvel[:, :]    = 0.0
        self.block_part2_angvel[:, :]    = 0.0
        self.block_sec_last_angvel[:, :] = 0.0
        self.block_last_angvel[:, :]     = 0.0

        # Set small and large gear root state
        parts_actor_ids_sim = torch.cat(
            (self.block_part1_actor_ids_sim, self.block_sec_last_actor_ids_sim,self.block_part2_actor_ids_sim), dim=0
        ).to(torch.int32)
        self.gym.set_actor_root_state_tensor_indexed(
            self.sim,
            gymtorch.unwrap_tensor(self.root_state),
            gymtorch.unwrap_tensor(parts_actor_ids_sim),
            len(parts_actor_ids_sim),
        )

        # Simulate one step to apply changes
        self.simulate_and_refresh()

    def _reset_inhand_part(self, before_move_to_grasp):
        """Reset root state of medium gear."""
        # world_to_gear_base_pos = torch.zeros_like(self.gear_medium_pos)
        # world_to_gear_base_quat = torch.zeros_like(self.gear_medium_quat)
        ...
        if before_move_to_grasp:
            ...
            ee_to_block_tip_pos_local = torch.zeros_like(self.fingertip_midpoint_pos)
            gear_half_height = self.asset_info_gears.shafts.height / 2.0  # half height of gear
            # print(f'👆###👆#')
            # print(f'gear_half_height  {gear_half_height}')        
            tuning_x = -0.0  # Positive values means far from robot
            tuning_y = -0.0 #positive means left
            tuning_z = 0.08 # Positive values means downward 

            # set medium_gear in the middle of gripper
            ee_to_block_tip_pos_local[:, 2] = -gear_half_height  + tuning_z 
            # ee_to_block_tip_pos_local[:, 1] = tuning_y
            # ee_to_block_tip_pos_local[:, 0] = + tuning_x +fine_tuning_x

            # cal medium_gear pos quat in whold frame
            world_to_block_tip_quat, world_to_block_tip_pos = torch_jit_utils.tf_combine(
                self.fingertip_midpoint_quat,  # Gripper current orientation
                self.fingertip_midpoint_pos,   # Gripper current position
                self.identity_quat,            
                ee_to_block_tip_pos_local       
            )

            # cal medium_gear to root position
            block_tip_to_base_local = torch.zeros_like(self.fingertip_midpoint_pos)
            block_tip_to_base_local[:, 2] = gear_half_height  

            world_to_block_base_quat, world_to_block_base_pos = torch_jit_utils.tf_combine(
                world_to_block_tip_quat,
                world_to_block_tip_pos,
                self.identity_quat,    
                block_tip_to_base_local  
            )
            # # ✨ flip 🐓 xyzw   
            quat_flip_x = torch.tensor([1, 0, 0, 0], device=self.device).unsqueeze(0).repeat(self.num_envs, 1)
            world_to_block_base_quat = torch_jit_utils.quat_mul(quat_flip_x, world_to_block_base_quat)

            # set medium_gear's  root state
            self.block_last_pos[:, :] = world_to_block_base_pos
            self.block_last_quat[:, :] = world_to_block_base_quat
        ...
        block_last_actor_ids_sim = self.block_last_actor_ids_sim.clone().to(
            dtype=torch.int32
        )
        self.gym.set_actor_root_state_tensor_indexed(
            self.sim,
            gymtorch.unwrap_tensor(self.root_state),
            gymtorch.unwrap_tensor(block_last_actor_ids_sim),
            len(block_last_actor_ids_sim),
        )
